// Taken from https://github.com/auth0/node-xml-encryption
...
function encryptKeyInfoWithScheme(symmetricKey, options, scheme, callback) {
  try {
    ...
    const certificate = pki.certificateFromPem(options.pem);

    const issuerArray = [`CN=${certificate.issuer.attributes.find(a => a.shortName === "CN").value}`,
    `OU=${certificate.issuer.attributes.find(a => a.shortName === "OU").value}`,
    `O=${certificate.issuer.attributes.find(a => a.shortName === "O").value}`,
    `L=${certificate.issuer.attributes.find(a => a.shortName === "L").value}`,
    `C=${certificate.issuer.attributes.find(a => a.shortName === "C").value}`,
    `emailAddress=${certificate.issuer.attributes.find(a => a.shortName === "E").value}`];

    const params = {
      encryptedKey:  base64EncodedEncryptedKey,
      serialNumber: utils.getSerialNumber(certificate),
      issuerData: issuerArray.join(', '),
